...
实际断卦先读 [断卦与条件审查流程](references/condition-review.md)：明确原问维度与时限 → 取用并对应人物物件 → 回查所选作者总纲、判断实际作用与例外 → 按原问分开吉凶、现状细节和应期 → 核对冲突与遗漏 → 逐项回答原问。条件表用于检查这条分析主线，不代替主线。普通单题用简洁记录，批量审计再保存完整字段；同一任务已核读的原文可复用。
...
Git安装未附带本地汇编文件时，调用`get_source('prompt:GLOBAL.md')`固定读取发行包中的同版全文，按其中链接读取如`prompt:global/liuyao_zixiu_dxj.md`、`prompt:study/PROMPT.md`及领域原文。这个入口不依赖检索排名，也不生成摘要。`has_more=true`必须按`next_offset`续读；校验失败或服务版本不支持时报告加载缺口。源码开发者可从当前数据库运行`python scripts/build_skill_prompts.py`生成同版文件。
