...
实际断卦先读 [断卦与条件审查流程](references/condition-review.md)，再完整读取 [全局断卦执行顺序与原文](references/source-prompts/FLOW.md)。固定执行顺序是：先定原问、来意、对象与时限 → 核时间与盘面、动变边界 → 取主用神并列世应和必要辅助角色 → 先以日月、旺衰、动变、元忌对用神的实际作用判主线吉凶 → 再补世应、领域规则和象法细节 → 最后才按已判主线取应期并作答。FLOW.md中的步骤标题只做原文阅读和核查路由，不能替代下方完整原文，更不能把步骤表当成跨书统一公式。条件表用于检查这条分析主线，不代替主线。普通单题用简洁记录，批量审计再保存完整字段；同一任务已核读的原文可复用。
...
Git安装未附带本地汇编文件时，先调用`get_source('prompt:FLOW.md')`固定读取发行包中的同版流程原文，再调用`get_source('prompt:GLOBAL.md')`，按其中链接读取如`prompt:global/liuyao_zixiu_dxj.md`、`prompt:study/PROMPT.md`及领域原文。这个入口不依赖检索排名，也不生成摘要。`has_more=true`必须按`next_offset`续读；校验失败或服务版本不支持时报告加载缺口。源码开发者可从当前数据库运行`python scripts/build_skill_prompts.py`生成同版文件。
...
- 形成结论前，先按原问和选定的世、应、用神候选列出关键待核条件，不限于已返回规则。日冲静爻须核作用力及后续生克；墓候选、主变卦游魂归魂须核事项与效力限制。对关键原文连同required_contexts先做条件对照：原句、适用对象、必要前提及且/或关系、例外 → 本盘具体爻位/层次/角色和实际字段 → 成立/不成立/未知。不同对象或状态分项；同爻多角色分别核查，不能只摘支持结论的一句。原文关系不清时保留解释分歧，不擅自补成逻辑公式。
