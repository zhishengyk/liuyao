...
实际断卦先读 [断卦与条件审查流程](references/condition-review.md)，再完整读取 [全局断卦流程与原文](references/source-prompts/GLOBAL.md)。固定执行顺序是：先定原问、来意、对象与时限 → 核时间与盘面、动变边界 → 取主用神并列世应和必要辅助角色 → 先以日月、旺衰、动变、元忌对用神的实际作用判主线吉凶 → 再补世应、领域规则和象法细节 → 最后才按已判主线取应期并作答。GLOBAL.md中的步骤标题只做原文阅读和核查路由，不能替代其下原书正文，更不能把步骤表当成跨书统一公式。条件表用于检查这条分析主线，不代替主线。普通单题用简洁记录，批量审计再保存完整字段；同一任务已核读的原文可复用。
