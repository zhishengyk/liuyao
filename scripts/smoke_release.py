...
async def main():
    env = {k: v for k, v in os.environ.items() if k not in ("LIUYAO_ROOT", "LIUYAO_DB", "PYTHONPATH")}
    ...
    with tempfile.TemporaryDirectory() as cwd:
        ...
        async with Client(params) as client:
            ...
            for kind in ("rule", "case"):
                result = await client.call_tool("search_knowledge", {"query": "工作 官鬼 用神", "kind": kind})
