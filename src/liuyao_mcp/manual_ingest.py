        ...
        if unit["kind"] == "case":
            ...
            for record in records:
                ...
                if selected_quality["status"] == "eligible" and not record["question"]["question_only"].strip():
                    # 多起卦同一事件：评审已判定同题多盘为一个事件（如“四盘同一行程不能作四个独立事件”）。
                    # 无显式 per-cast 质量、且事件内其他盘有原问时，共享事件原问；显式判为 eligible 的盘仍必须自带问题。
                    if cast_quality is None and len(records) > 1:
                        event = next((r for r in records if r["question"]["question_only"].strip()), None)
                        if event is not None:
                            record["question"] = dict(event["question"])
                    if not record["question"]["question_only"].strip():
                        raise ValueError(f"eligible case requires a question at the selected input stage: {source['source_id']} unit={unit['unit_id']} case={record['case_id']}")
                if selected_quality["status"] == "eligible" and not record["parts"]["feedback"]["exact_text"].strip():
                    raise ValueError(f"eligible case requires actual feedback for the selected cast: {source['source_id']} unit={unit['unit_id']} case={record['case_id']}")
