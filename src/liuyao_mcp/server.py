...
INSTRUCTIONS += "理法和卦例先用get_topics识别大类和小类，再传topic/subtopic检索；include_common控制是否补充公共理法。同小类优先，不足时返回父类及明确标注的未知候补。classification为自动标注，不当成人工金标。build_chart的patterns按书中规则给出十二长生、刑害、反吟、隔山化爻等盘面前提及source_rule_id；先查原文条件，不能把命中格局当作事件或吉凶结论。可显式提供yongshen_positions作为待核实的取用候选，程序不自行选用神。"
INSTRUCTIONS += "检索结果的page_reviews区分机器底稿与逐字校订页；有page:来源ID:PDF页码时，可用get_source读取已核准整页文字，保留unclear原图疑点。普通证据的结构化字段不因整页校订而自动变成已复核。include_common仅允许公共规则参选，关键通用条件需要时另查，不强制固定配额。"
...
def build_chart(line_values: list[StrictInt], cast_time: str | None = None, month_branch: str | None = None, day_ganzhi: str | None = None, timezone: str = "Asia/Shanghai", question: str | None = None, yongshen_positions: list[StrictInt] | None = None) -> dict[str, Any]:
    """排盘。line_values按初爻到上爻，统一用0老阴、1少阳、2少阴、3老阳。给完整时间或历史月支+日干支。display.markdown包含六神、伏神、本变卦、动爻和世应，可直接展示。patterns给出结构格局与来源，命中前提不等于断事结论；yongshen_positions为调用方已查依据的用神候选爻位1..6。"""
...
def search_knowledge(query: str, kind: Literal["rule", "case"] = "rule", method: Literal["all", "lifa", "xiangfa"] = "all", topic: str | None = None, author: str | None = None, features: dict | None = None, limit: int | None = None, exclude_ids: list[str] | None = None, exclude_case_ids: list[str] | None = None, max_chars: int = 40000, retrieval_mode: Literal["bm25","hybrid","hybrid_rerank"] | None = None, outline_ids: list[str] | None = None, subtopic: str | None = None, include_common: bool = True) -> dict[str, Any]:
    ...
    try:
        return retrieve(query, kind, method, topic, author, features, limit, exclude_ids, exclude_case_ids, max_chars,retrieval_mode=retrieval_mode,outline_ids=outline_ids,subtopic=subtopic,include_common=include_common)
    except (ValueError,FileNotFoundError) as exc:
        raise ToolError(str(exc)) from exc
...
def get_source(evidence_id: str, context_lines: int = 0, offset: int = 0, max_chars: int = 40000, text_version: Literal['corrected','original'] = 'corrected') -> dict[str, Any]:
    """按evidence_id或目录node_id回查原文。返回目录路径、关联案例及预算内的导语outline_context；遗漏导语可另查。context_lines扩前后文；has_more时用next_offset续取。OCR的page_reviews提供已逐字核对页的ID（page:来源ID:页码），可另查整页校订稿；普通证据的text_version=original回查修订前文字。未核对页不能作为校订稿返回。"""
    try:
...
def main():
    ...
    if args.self_check:
        ...
        assert topics and get_topics('relationship')['items']
        assert len(chart['patterns']['combination_checks'])==18
        assert get_source('page:liuyao_lifa_jinjie:18')['text_version']=='visually_reviewed_page'
        ...
        print(json.dumps({"version": __version__, "status": "passed", "retrieval": "bm25", "counts": counts, 'outline_books': len(books), 'topic_roots':len(topics)}))
        return
