...
日期：2026-09-09。状态：核心服务已实现并安装；两端界面调用待用户手动验收。实际完成情况和证据见 [实施验收](实施验收.md)。

目标：交付一个可接入 ChatGPT 桌面端和 VS Code 内 Codex 扩展的“六爻助手”。底层通过 MCP 提供准确的排盘数据、可追溯的六爻理法和相似卦例，由接入端 AI 根据证据完成分析。

第一版交付包含本地 MCP 服务、安装配置和薄插件包：三个工具提供能力，插件内的使用指引帮助 AI 按理法调用。用户安装或配置一次，之后在已有 AI 客户端里直接提问。按用户确认的 6 份现有文件自动处理完整正文，不设置人工检索、逐条挑选或人工标注的前置流程。预计 1 名开发者用 2–3 周完成首版；OCR 结构解析和客户端联调是主要不确定项。
...
- OCR 核对表保存了部分页面的置信度和原图路径，但本工作区缺少原图。未知质量不能补成虚构的高置信度。
...
MCP 服务负责计算和提供证据。接入端 AI 负责理解占问、选择用神、组织分析和表达结论。服务端在线流程不再调用一个生成式大模型，也不需要生成模型的 API Key；后续本地 embedding/reranker 模型是独立的检索组件。
...
- 六爻从初爻到上爻，即从下向上输入；6=老阴、7=少阳、8=少阴、9=老阳。
...
三个工具均采用只读查询语义。原文回查通过证据 ID 找已登记资料，不暴露任意文件读取接口。
...
关键词匹配负责明确术语，后续向量匹配负责不同问法的语义联系，结构匹配负责盘面条件。案例的 Symbolic Features 从盘面计算和作者解释分别生成，注明字段来源并绑定具体爻位；未知字段不当作已确认不匹配。
...
首批 6 份资料阶段将向量保存为 NumPy 矩阵，直接计算相似度。增加专用向量数据库的条件是实测内存、延迟或并发成为问题。embedding 一次离线计算并按文本哈希缓存，避免重复处理。

只有出现“召回里有正确证据，前几名却排错”的稳定问题，才接 `BAAI/bge-reranker-v2-m3`，对前几十条做一次重排。该模型以 query/passage 对为输入，适合作为可直接试验的基线。[官方模型卡](https://huggingface.co/BAAI/bge-reranker-v2-m3)
...
| MCP | 官方 `mcp` Python SDK，锁定验证过的版本 | 直接声明三个工具及返回类型 |
...
| 检索 | FTS5/BM25，随后按评测增加 BGE dense | 先获得可解释基线 |
...
共享给其他设备或接入云端 ChatGPT 时再部署 Streamable HTTP；远程版本补认证和访问限制。本地服务的 stdout 仅用于协议消息，日志写 stderr。
...
| 用户入口 | 第一版接入 | 验收要求 |
| --- | --- | --- |
| ChatGPT 桌面端 | Settings → MCP servers → Add server，填写本地启动命令；也提供插件安装入口 | 重启后能发现三个工具，从聊天完成理法检索与原文回查 |
| VS Code 中的 Codex | 齿轮菜单 → MCP servers → Add server；同一主机可复用上述配置 | 重启扩展后能发现并实际调用同一组工具 |
| 插件目录中的“六爻助手” | 将服务连接、名称说明和使用 skill 打包，先从本地 marketplace 安装测试 | 用户能安装、启用，在新会话中触发正确工具；按实际客户端分别测试目录入口 |
