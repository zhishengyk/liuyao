...
**最新发布约束（2026-09-09，用户确认）：下载预建数据库后，本 MCP 的排盘和检索离线运行；用户不需要重新切片、编码资料或安装 BGE 大模型。** 建库与使用分开：开发端处理六份资料并发布数据库，用户端通过 stdio 查询本地数据。用户已接受简化Windows安装步骤，并要求支持自动更新；根据后续讨论，优先通过GitHub发布与Codex原生目录/安装机制同步版本，自动触发方式按客户端能力验证。检索进程继续使用本地版本，断网或候选准备失败不影响已安装版本，当前对话不热切换程序和数据库。自动同步尚未实现，现有源码/uvx入口仍按README手动更新。接入端 AI 的运行方式由客户端决定。
