...
日期：2026-09-09。本文保留系统设计与开发过程中的取舍；当前0.5.0安装、起卦和使用方式以[README](../README.md)为准，实际完成情况见[实施验收](实施验收.md)。GitHub插件安装/更新入口已提供，独立神经检索仍属实验。

目标：交付一个可接入 ChatGPT 桌面端和 VS Code 内 Codex 扩展的“六爻助手”。底层通过 MCP 提供准确的排盘数据、可追溯的六爻理法和相似卦例，由接入端 AI 根据证据完成分析。

**知识组织方向（2026-09-10）：** 理法按占问大类／小类组织，并补充适用的公共理法；象法不增加事项分类体系，改用《六爻象法进阶》上下册的原书目录、实际场景和关键爻线索定位用法与案例。已核对两册正文目录并发现现有chapter的边界错误，具体目录、结构字段、查询流程与实施顺序见[象法目录与场景索引方案](象法目录与场景索引方案.md)。该索引已在0.5.0实现：用PDF核对目录，修复边界，新增get_outline和象法场景查询；理法的大类／小类体系仍是后续规划。
...
四个工具均采用只读查询语义。原文回查通过证据 ID 找已登记资料，不暴露任意文件读取接口。
